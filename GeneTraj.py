# -*- coding: utf-8 -*-
# 轨迹导出脚本 (Categorical 436-dim action space)
# 输出字段：episode, t, action, reward, remaining_plays, remaining_discards,
...
# ====== 扑克牌元组 (rank:int 1-13, suit:'H/D/C/S') 转字符串 ======
...
@torch.no_grad()
def policy_act(net, obs_np, device):
    """
    Categorical(436) 采样策略：
      - 构建 valid action mask
      - 从 436 维 logits 中采样
      - 转换为 (a_type, card_mask_52)
    """
    x = torch.as_tensor(obs_np, dtype=torch.float32, device=device).unsqueeze(0)
    action_logits, _ = net(x)
    ...
    with open(csv_path, "w", newline="", encoding="utf-8") as fcsv:
        ...
        for ep in range(1, episodes + 1):
            ...
            while not done and t < max_steps_per_ep:
                ...
                hand_before = list(env.hand)
                a_type, card_mask_52 = policy_act(net, obs_np, device)
                action_name = "play" if a_type == 1 else "discard"

                # 用 card_mask_52 在 hand_before 上取子集
                cards_involved = [
                    card for card in hand_before
                    if card_mask_52[env._card_index(card)] == 1
                ]

                next_obs, reward, done, _ = env.step((a_type, card_mask_52))
